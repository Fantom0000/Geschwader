...
int main()
{
    ...
    while (true) {
        ...
        if (GameState::get().players.size() == pl.num_players && GameState::get().current_player_idx == bot.idx) {
            ...
            for (auto it = GameState::get().vehicles.begin(); it != GameState::get().vehicles.end(); ++it) {
            ...
            for (int i = 0; i < GameState::get().vehicles[bot.idx].size(); ++i) {
                actions[i] = GameState::get().vehicles[bot.idx][i]->step(map_matrix, tanks_can_be_attacked);
                //if (i == 0) {
                ...
                //    }
                //    map_dist[code(GameState::get().vehicles[bot.idx][i]->position, rad)] = INT_MAX;
                //    points_move.push(code(GameState::get().vehicles[bot.idx][i]->position, rad));
                ...
                //        action.vehicle_id = GameState::get().vehicles[bot.idx][i]->tank_id;
                //        action.target = point_to_move;
                //        map_matrix[code(point_to_move, rad)] = MapCode::FRIENDLY_TANK;
                //        map_matrix[code(GameState::get().vehicles[bot.idx][i]->position, rad)] = MapCode::NOTHING;
                ...
                //            if (can_attack_tank(GameState::get().vehicles[bot.idx][i], *victim, map_matrix, rad)) {
                ...
                //        }
                //        else {
                //            cout << (json)GameState::get().vehicles[bot.idx][i].position << " attacked " << (json)tank_to_attack->position << endl;
                //            action.vehicle_id = GameState::get().vehicles[bot.idx][i].tank_id;
                ...
                //    if (GameState::get().vehicles[bot.idx][i].position == point_to_move) {
                ...
                //        continue;
                //    }
                //    if (distance(GameState::get().vehicles[bot.idx][i].position, point_to_move) <= 2 && center_is_occupied){
            ...
            /*for (auto& bot_tank : GameState::get().vehicles[bot.idx]) {
                Tank* victim = nullptr;
                cout << "try attack" << endl;
                for (const auto& enemy : can_be_attacked) {
                    for (auto& vehicle : GameState::get().vehicles[enemy]) {
                ...
                if (victim != nullptr) {
                    ...
                    cout << "try move" << endl;
                    vector<Point> way = find_min_way(bot_tank.position, mm.base, mm.obstacle, rad, map_matrix, GameState::get().vehicles[bot.idx]);
